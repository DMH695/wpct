...
public class WechatUserServiceImpl implements WechatUserService {
    ...
    @Override
    public ResultBody getWechatUserInfoAndHouseInfo(int pageNum, int pageSize) {
        JSONObject result = new JSONObject();
        PageHelper.startPage(pageNum,pageSize);
        List<WechatUser> info = wechatUserMapper.getWechatUserInfoAndHouseInfo();
        ...
        if (info == null){
            ...
            for (int i = 0; i < info.size(); i++) {
                ...
                result.put("buildName",info.get(i).getBuildName());
                result.put("roomNum",info.get(i).getRoomNum());
            }
        }
